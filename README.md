...
```bash
python3 -m scripts.build_mcpb     # produces dist/codetalk-0.2.2.mcpb
```

Drag `codetalk-0.2.2.mcpb` into your client's extension-install entry point, and pick a project root during installation. It exposes **7 tools** (all marked `readOnlyHint: true`, so Claude Code / Cursor can auto-approve them without a confirmation prompt):
...
```bash
cursor --install-extension vscode-codetalk-0.2.2.vsix   # Cursor
code --install-extension vscode-codetalk-0.2.2.vsix      # VS Code

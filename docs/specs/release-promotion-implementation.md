...
**Goal:** Add a manual, dry-run-first 0.3.1 release workflow that builds once,
...
- This plan originally targeted `0.2.0`. Earlier identities are retained only
  in the historical run evidence below; the active release identity is `0.3.1`.
...
- The corrected `v0.2.0` promotion run `29880629420` passed the repository,
  candidate, tag, and OIDC gates. PyPI then returned HTTP 400 because the exact
  `0.2.0` filenames had been uploaded and deleted previously; PyPI permanently
  reserves those names even though its public version endpoint returns 404.
  Release publication and Pages remained skipped, and the GitHub Release stayed
  a hidden draft. The first recovery advanced the active identity to `0.2.1`.
- The `0.2.1` rehearsal `29881493075` passed every non-publishing gate. Its
  authorized promotion `29882535367` then passed OIDC and failed on the same
...
- The `0.2.2` rehearsal `29884029536` passed every non-publishing gate. Its
  ...
  patch versions, recovery advanced directly to `0.3.0` instead of probing
...
- Release identity is fixed to PyPI distribution `hukair-codetalk`, version
  `0.3.1`, and tag `v0.3.1`. The CLI command, import package, MCP identity, and
  editor branding remain `codetalk` / CodeTalk.
...
- Create `docs/releases/v0.3.1.md`: make the notes suitable for both the tag and
...
0.3.1 release contains exactly the expected wheel and sdist and both public
...
jobs:
  ...
  candidate:
    ...
    steps:
      ...
      - run: python -m scripts.release_promotion validate-candidate dist docs/releases/v0.3.1.md
...
`publish-release` uses `gh release edit v0.3.1 --draft=false --latest`, then
requires `gh release verify v0.3.1`, `gh release verify-asset` for all six files,
...
runtime assets, and install `hukair-codetalk==0.3.1` from public PyPI into a clean
...
git commit -m "ci(release): add guarded 0.3.1 promotion" \
...
Require the release heading `# CodeTalk 0.3.1`, the four known limitations,
...
Retitle the notes `# CodeTalk 0.3.1`, preserve the product workflow and honest
...
Add an exact `0.3.1 Promotion` section recording the current blocked state and
the owner actions:
...
`gh workflow run release.yml --ref v0.3.1 -f publish=true` command, public PyPI
...
```bash
git add -- docs/releases/v0.3.1.md RELEASE_CHECKLIST.md tests/test_release_candidate.py docs/specs/release-promotion-implementation.md
git commit -m "docs(release): define 0.3.1 promotion runbook" \

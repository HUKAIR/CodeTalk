...
- **补 evidence 会话原话覆盖 ✅ 已修复**:`enrich` 改为扫真实会话对齐 + 新增零-LLM `backfill_evidence`(`cache.set_narrative_evidence` 只加 evidence、不动 LLM why/decisions,守 immutable)。`vibetrace enrich --no-llm` 实测 **evidence 0→68**(覆盖 92.4%)。护城河第三支柱「引真实会话原话」立起。

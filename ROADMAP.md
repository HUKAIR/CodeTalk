...
- **MCP server 扩面 ✅ 已落地**(PR #127/#128):4→**7 工具**(新增 `vibetrace_drift`/`vibetrace_prompts`/`vibetrace_adr`,全零-LLM);模块拆分 `mcp_server.py`(115 行 serve loop)+ `mcp_tools.py`(217 行工具定义+dispatch),守 300 行红线;全工具英文描述 + `readOnlyHint: true` annotations(Claude Code/Cursor 自动批准);manifest/README 同步更新。45 测试全过。
...
- ~~待办(v2):指令→AI 回应→commit 三段接地~~ ✅ 已落地(PR #131):prompts_view 展示 AI response excerpts(session 已抓的 assistant text >80 chars),MCP `vibetrace_prompts` 同步受益。剩 console 第五视图(视觉层,待需求)。
...
usage.log 漏项目绝对路径)。~~**建设缺口:无 `--no-llm`/本地 provider 硬开关**~~ ✅ 已落地:CLI `--no-llm` 各命令均有、config `"no_llm": true`、环境变量 `VIBETRACE_NO_LLM=1`、Ollama provider 内置。
...
- **产品事实修正**:跨项目聚合已 merged(distinct_projects/build_overview/watch/self_report)= 增量非从零;~~本地/可关 LLM 选项是真建设缺口~~ ✅ 已落地(`--no-llm` + Ollama provider)。

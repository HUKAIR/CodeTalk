...
>
> 盲测 N=5、本仓、人工判读，不是人群结论。接地覆盖上限 100%（199/199 commit 全有叙事，`grounding_hitrate.py` 可复跑；源码行级可达率 100%）。vibetrace 找的是「当初到底说了什么、为什么这么写」，不保证代码正确——源记录本身可能有误。100% 是本仓 dogfood 结果，不是一般性保证。完整方法见 `docs/discovery/` 下对照卡和盲测文档。

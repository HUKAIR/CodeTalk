...
```bash
cd ~/Github/CodeTalk
python3 -m scripts.build_mcpb        # → 生成 dist/codetalk-0.2.0.mcpb
```
...
unzip -q dist/codetalk-0.2.0.mcpb -d "$vt"
printf '%s\n' \
...
2. 点 **Install Extension…** 选 `codetalk-0.2.0.mcpb`,或直接把文件拖进窗口
3. 安装界面会要一个 **项目路径(project)** → 填某个 git 仓根目录(如 `~/Github/你的项目`)
4. 开启扩展 → 新建对话,工具列表里应出现 7 个 `codetalk_*`

_（预期界面:Claude Desktop → Settings → Extensions:安装 codetalk-0.2.0.mcpb,填 project 路径,启用后 7 个工具出现）_

...
- **决策面包屑**: commit message 里的普通文本行,例如 `Vibe-Decision: ...`
  和 `Vibe-Watch: ...`。它记录「为什么这样改」和「以后要回头验证什么」。这不是 AI
  专属功能,只是可被 git 保存、可被 CodeTalk 读取的提交说明。
- **`enrich` / 富集**: 可选的历史补全步骤。它会让 LLM 读取旧 commit 和附近的本地会话摘要,
  生成决策叙事。只有旧提交里还没有决策面包屑时才需要。
- **接地 / grounding**: 回答必须能指回真实 commit、决策面包屑或会话原话。没有证据,
  就不让模型自信发挥。
- **零-LLM / 零出网**: 只做本地确定性查找,不调用模型。加 `--no-llm` 后,连可选的 LLM
  调用也关闭,项目数据留在本机。
- **时间胶囊**: `Vibe-Watch` 记录的风险会在未来某天重新出现在报告里,提醒你验证当初的担心是否成真。
- **MCP**: 一种把 CodeTalk 命令暴露给 Claude Code、Cursor、Codex 等 AI 编码客户端的标准接口。
- **理解债**: 最近改动多、带风险、或还没认真回看的文件/决策;它们值得在开工前优先补读。

> **诚实边界:** 盲测 N=5、本仓、人工判读，不是人群结论。覆盖率取决于 `enrich`：本仓（全量补全）100%（220/220）；另一本地 605-commit 仓**未 enrich** 时仅 0.3%，跑完 `codetalk enrich` 后可达 ~100%。未 enrich 且无面包屑时，blame 只显示 commit subject——与 `git log` 接近。请在你自己的仓上跑 `grounding_hitrate.py` 度量。CodeTalk 找的是「当初到底说了什么、为什么这么写」，不保证代码正确——源记录本身可能有误。覆盖率数字可用 `grounding_hitrate.py` 在你自己的仓复现;盲测方法见 `python3 scripts/blind_test.py`;护城河对照写作见 `docs/moat-real-records-vs-inference.md`。
...
**诚实冷启动:** 没有面包屑、也没跑 `enrich` 的仓库只显示 commit subject——和 `git log` 一样。价值来自面包屑(免费、就在 git 里)或 `enrich`(需要 key)。本仓两者都有,所以上面 30 秒 demo 内容丰富。你的仓从空开始,随用随填。

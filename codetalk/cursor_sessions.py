"""Tolerant parser for Cursor's local AI sessions (composer) → codetalk
...
from .config import redact_secrets, CODETALK_DIR
...
NOTICE_SENTINEL = CODETALK_DIR / ".cursor_notice_shown"
...
def maybe_notice():
    ...
    try:
        ...
        log.warning("已启用 Cursor 会话源:将读取本地 Cursor 会话(只读、不出本机);"
                    "可在 ~/.codetalk/config.json 的 sources 移除 \"cursor\" 关闭。")

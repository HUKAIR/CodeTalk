"""Tolerant parser for OpenAI Codex CLI local sessions → codetalk session
...
from .config import CODETALK_DIR, redact_secrets
...
NOTICE_SENTINEL = CODETALK_DIR / ".codex_notice_shown"
...
def maybe_notice():
    ...
    try:
        ...
        log.warning("已启用 Codex 会话源:将读取本地 Codex rollout 会话(只读、不出本机);"
                    "可在 ~/.codetalk/config.json 的 sources 移除 \"codex\" 关闭。")

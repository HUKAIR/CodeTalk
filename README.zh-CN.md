...
要求 Python 3.11+。PyPI 发行名是 `hukair-codetalk`，安装后的命令仍是
`codetalk`。唯一列出的替代安装器是 `uv tool install hukair-codetalk`。安装后先跑
...
```bash
python3 -m scripts.build_mcpb     # 产出 dist/codetalk-0.3.1.mcpb
```

构建出的 `codetalk-0.3.1.mcpb` 可以直接拖进客户端安装;在 GitHub 提供可下载 Release 前,
...
- **自托管**:单镜像 Docker(见 `Dockerfile`:`docker build -t codetalk .` → `docker run`)。

...
Approved direction for preparing the CodeTalk 0.3.1 public release. This design
...
The 0.3.1 workflow is release-specific:

- PyPI distribution: `hukair-codetalk`;
- package version: `0.3.1`;
- tag: `v0.3.1`;
- release notes: `docs/releases/v0.3.1.md`;
- expected artifact names come from `scripts/release_artifacts.py`.

The installed command and Python import remain `codetalk`; MCP and editor
artifacts keep the CodeTalk brand. Only the collision-prone PyPI distribution
identity changes.

Dry runs build the immutable commit that started the workflow. Promotion must
be manually dispatched from the exact `v0.3.1` tag after that tag contains the
...
- all package metadata reports version `0.3.1`;
...
- promotion conditions and fixed 0.3.1 identity are present;
...
- create the verified annotated `v0.3.1` tag at the recorded source commit;
- protect `v0.3.1` against update and deletion with a repository tag ruleset;

...
- **MCP 对齐参数词表 + 集成层定位**:保留独占 `vibetrace_*` 工具名,加可选入参别名(sha/path/startLine/endLine/pullNumber,owner/repo 收下即忽略),定位「GitHub MCP 之上的零-LLM why 增强层」。⚠ 不可改名成 get_diff/get_commits(GitHub MCP 真名是 get_commit/list_commits/pull_request_read,改名会撞名+丢 why 护城河)。[R2 · 集成,对抗检验修正]
...
- **接地质量可验收**:逐句/逐段归因(哪句有据/推断)+ groundedness eval 门(RAGAS/DeepEval,字符串恒等口径)。现仅粗徽标。[R1 SO/Uber + R2 Trust-Score]
...
**待第三轮 deep-research 补的证据盲区(进行中,wo7z62ib2)**:① 变现/定价(开源+云、按席位 vs 用量、团队版触发点)② 本地模型能力上限基准 ③ 接地 eval 框架适配(RAGAS/TruLens/DeepEval)④ MCP 集成 vs 竞争实践。

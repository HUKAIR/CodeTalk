# vibetrace MCP 一键装 + 验证指南

把 vibetrace 的零-LLM 接地能力(`ask`/`blame`/`graph`/`search`/`drift`/`prompts`/`adr`)装进 Claude Desktop /
Claude Code / Cursor 等 MCP 客户端,在 agent 工作流里直接问「这段代码当初为什么这么写」。
全程 **stdio 同机直连、数据不出本机**。

> 📷 各步骤下方斜体「预期界面」描述你装好后应看到什么(截图待补,文件名清单见 [`docs/images/README.md`](images/README.md))。
...
- 想用 `ask` 的 LLM 综合:跑过 `vibetrace init` 配好 key(不配也行,`ask` 自动降级为确定性检索)。

## 1. 构建包

```bash
cd ~/Github/CodeTalk
python3 -m scripts.build_mcpb        # → 生成 vibetrace.mcpb
```

## 2.(推荐)命令行自检,先确认包本身没问题

```bash
rm -rf /tmp/vt && mkdir -p /tmp/vt && unzip -q vibetrace.mcpb -d /tmp/vt/x
printf '%s\n' \
 '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-11-25"}}' \
 '{"jsonrpc":"2.0","id":2,"method":"tools/list"}' \
 | PYTHONPATH=/tmp/vt/x/server python3 -P -m vibetrace mcp-serve --project ~/Github/CodeTalk
```

**通过标准**:输出两行 JSON —— 第一行含 `"serverInfo":{"name":"vibetrace"}`,第二行含 7 个
`vibetrace_*` 工具。看到 = 包 OK,可进客户端。(`-P` 屏蔽当前目录,保证用的是包内源。)
...
4. 开启扩展 → 新建对话,工具列表里应出现 7 个 `vibetrace_*`

_（预期界面:Claude Desktop → Settings → Extensions:安装 vibetrace.mcpb,填 project 路径,启用后 7 个工具出现）_
...
```json
{
  "mcpServers": {
    "vibetrace": {
      "command": "python3",
      "args": ["-m", "vibetrace", "mcp-serve", "--project", "/abs/path/to/your/repo"]
    }
  }
}
```
...
_（预期界面:Claude Code:.mcp.json 配置 + claude mcp list 显示 vibetrace connected）_
...
_（预期界面:Cursor Settings → MCP:vibetrace 已连接、显示工具数）_
...
- 让它**列出工具** → 7 个 `vibetrace_*` 在。
- `vibetrace_blame`,target 填 `vibetrace/llm.py:27-33` → 返回触达这些行的 commit + 决策史(**零 LLM**)。
- `vibetrace_search`,关键词 ≥3 字(如「脱敏」)→ 相关 commit 的真实 why / 原话锚点。
- `vibetrace_ask` 问「这段为什么这么写」→ 接地回答(配了 key 才有 LLM 综合,否则确定性检索)。
- 有真实内容 = 端到端通。

_（预期界面:agent 实际调用 vibetrace_blame / vibetrace_ask 的接地返回）_
...
- **MCP 场景**(mcp-serve 不吃 `--no-llm` flag,用环境变量或 config):
  - 在 step B/C 的服务器配置里加 `env` 块:
    ...
    "vibetrace": {
      "command": "python3",
      "args": ["-m", "vibetrace", "mcp-serve", "--project", "/abs/path/to/your/repo"],
      "env": { "VIBETRACE_NO_LLM": "1" }
    }
    ```
  - 或在 `~/.vibetrace/config.json` 设 `"no_llm": true`(对所有命令 + MCP 全局持久生效)。
- **CLI 场景**:`vibetrace digest/ask/course --no-llm`。
...
| 现象 | 多半原因 | 处理 |
|---|---|---|
| 工具不出现 / server 起不来 | `python3` 不是装了 vibetrace 的那个,或不在客户端 PATH | 用绝对路径 python(见 step 3 坑) |
| `ask` 说「材料不足」/ 没综合 | 没配 key,或该文件无 commit 历史 | 用 blame/graph/search(不需 key),或 `vibetrace init` 配 key |
...
| 工具 | 作用 | LLM |
|---|---|---|
| `vibetrace_ask` | 就一段代码接地提问「当初为什么这么写」 | 配 key 用云端 LLM 综合;无 key 降级确定性 |
...
| `vibetrace_adr` | ADR 导出:从真实 git 历史自动生成架构决策记录(MADR/Nygard/CycloneDX) | 零 LLM |

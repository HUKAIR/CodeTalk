"""Config loading and shared secret redaction for codetalk."""
import copy
import json
import logging
import os
...
CODETALK_DIR = Path.home() / ".codetalk"
# 改名迁移(vibetrace→codetalk):旧 ~/.vibetrace 还在、新目录不存在时,一次性搬过来,
# 不丢用户的 cache.db(已富集叙事)+ config.json(含 key)。容错:迁移失败降级用旧目录。
_LEGACY_DIR = Path.home() / ".vibetrace"
if not CODETALK_DIR.exists() and _LEGACY_DIR.is_dir():
    try:
        _LEGACY_DIR.rename(CODETALK_DIR)
    except OSError:
        CODETALK_DIR = _LEGACY_DIR              # 搬不动(跨盘/权限)→ 仍读旧目录,不崩
CONFIG_PATH = CODETALK_DIR / "config.json"
CACHE_DB_PATH = CODETALK_DIR / "cache.db"
USAGE_LOG_PATH = CODETALK_DIR / "usage.log"

DEFAULTS = {
    "vault_path": str(Path.home() / "codetalk-reports"),
    "provider": "deepseek",
...
def load_config():
    ...
    if os.environ.get("CODETALK_NO_LLM"):       # 环境变量硬关 LLM(数据不出本机),一次性覆盖
        cfg["no_llm"] = True
    return cfg

...
_FORMAT_EXPECTED_KEYS = {"type", "timestamp"}
...
def _parse_file(path, is_subagent=False):
    ...
    with open(path, encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            if not _format_warned and not _FORMAT_EXPECTED_KEYS.issubset(obj.keys()):
                log.warning("Claude Code session format may have changed — "
                            "expected keys %s, got %s in %s. "
                            "Falling back to best-effort parsing.",
                            _FORMAT_EXPECTED_KEYS, set(obj.keys()), path.name)
                _format_warned = True
            rtype = obj.get("type")

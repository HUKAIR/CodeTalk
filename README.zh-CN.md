...
- **决策记录行**(内部曾叫 breadcrumbs / 面包屑): git commit message 里的两类可选行。
  `Vibe-Decision: ...` 写「这次为什么这样改」,`Vibe-Watch: ...` 写「以后要验证的风险」。
  它们存在 git commit 里,可以由人、git hook 或 AI coding agent 写入;CodeTalk 在
  `blame`、`ask`、`graph`、`digest` 里读取它们。
...
- **接地证据 / grounding evidence**: CodeTalk 能引用的真实材料,包括 commit message、
  `Vibe-Decision` / `Vibe-Watch` 行、本地会话原话摘录。没有证据时,CodeTalk 应该退回到
  “列出找到的材料”,而不是让模型自己编答案。
...
> **诚实边界:** 盲测 N=5、本仓、人工判读，不是人群结论。覆盖率取决于决策记录行 + `enrich`：全新克隆本仓可复现 **~59% 真实接地率(114/194 commit——只算逐字面包屑 + 会话原话锚点,排除 LLM 生成叙事,测于 2026-07-02)**——跑 `python3 scripts/grounding_hitrate.py .` 看实时数(它把真实接地率标星;含 LLM 叙事的较松上限印在下面,**不当**可信度背书);全量 `codetalk enrich` 补全后较松覆盖可达 ~100%。另一本地 605-commit 仓**未 enrich** 时仅 0.3%，跑完 `codetalk enrich` 后可达 ~100%。未 enrich 且无决策记录行时，blame 只显示 commit subject——与 `git log` 接近。请在你自己的仓上跑 `grounding_hitrate.py` 度量。CodeTalk 找的是「当初到底说了什么、为什么这么写」，不保证代码正确——源记录本身可能有误。覆盖率数字可用 `grounding_hitrate.py` 在你自己的仓复现;盲测方法见 `python3 scripts/blind_test.py`;护城河对照写作见 `docs/moat-real-records-vs-inference.md`。
...
# 本仓自己使用 CodeTalk——多数 commit 都有决策记录行(101/183)
# (commit message 里的 Vibe-Decision / Vibe-Watch 行)。
...
| `install-hook` | 装 git 钩子:手写 commit 时在编辑器里提示留 `Vibe-Decision`/`Vibe-Watch` 决策记录行 | `codetalk install-hook` |
| `install-agent-seed` | 把决策捕获约定植入项目 `CLAUDE.md` + `AGENTS.md`,让 **AI coding agent**(Claude / 其他)提交时自动留决策记录行(**写时捕获 > 事后从 diff 反推**) | `codetalk install-agent-seed` |
...
编辑器里会自动提示这两行——填了就保留在 commit message 里,不填会被 git 剥除。git 钩子不随仓库版本控制,
**每个 clone 各装一次**。

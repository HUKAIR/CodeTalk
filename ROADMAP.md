...
**Now(低成本 / 红线内 / 高 ROI)**
- **「AI 声称 vs git 实际」偏差报告(R7,可建)**:用现成 `sessions.files_written`(AI 声称改的文件)∖ git diff 实际文件 → 确定性零-LLM 偏差(「声称动 N、实际 M、P 个声称却没碰」「说加测试但无 test 文件」「说改但 diff 空」)。**只报字面可数缺口、禁语义完成度 %**(语义需模型,违零-LLM 护城河)。直击「AI 说了没做」痛点;竞品 Agentplane 不算偏差=空白缝。走完整 brainstorm→对抗审→SDD(需额度)。[R7]
- **引用核验加 hover 预览 + 点击跳真实源**(对齐 GitLens hover-card 范式;现仅可点开)。[R2 · UX]
- **文件树「接地入口」视图**:在 `report`/`console` web 里加项目文件树,用 `git status --porcelain` 确定性 XY 码零-LLM 高亮新增/修改/未跟踪;**定位为导航入口**(点变更文件 → 接地追问「这文件当初为什么改」),不是 diff 查看器(纯 what 撞 GitLens/DeepWiki、off-mission)。默认 tooltip/文字不靠单字母+颜色(可访问性,借 VS Code#123103 教训)。零-build 单文件 vanilla-JS,红线内。[R5 · UX]
...
- **面包屑 schema 借 Lore 扩**:加「被否决的备选 / agent 指令 / 验证元数据」+ git-adr 式 commit-link 行级锚定。[R2 · 能力,红线内 · 写侧加 trailer 有摩擦风险,defer/仅 agent 自动留]
...
- **只读收割命令历史/会话作接地素材(红线内版)**:把现有会话捕获扩展到只读 shell 命令历史(`~/.*_history`)+ Cursor 会话,严格本地 + 落盘前脱敏 + 可关。⚠ 拒绝可执行内嵌终端(Cursor 一串 RCE/提示注入 CVE 证为不可消除攻击面)+ OS 级「思考模式」全量捕获(撞 Pieces 腹地;N=1 不为「记录」付费——捕获是手段、接地 why 才是目的)。[R5 · 能力/隐私]

...
vault/vibetrace/<slug>/<sha7>.md,链回当天日报 [[{date}-{project}]];Obsidian 据此在日报侧
...
def _note(sha, date_str, project, subject, decisions):
    ...
    lines.append("")
    lines.append("<!-- vibetrace 自动产出(零 LLM,从真实 commit 决策挖);勿手改,会被覆写 -->")
    return "\n".join(lines) + "\n"
...
def emit_decision_notes(commits, project, vault_path, pkey):
    ...
    if not vault_path:
        return 0
    try:
        out_dir = Path(vault_path).expanduser() / "vibetrace" / _slug(project, pkey)
